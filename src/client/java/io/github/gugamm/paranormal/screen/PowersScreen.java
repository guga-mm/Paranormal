...
import io.github.gugamm.paranormal.screen.elements.button.PowerSlotButton;
import io.github.gugamm.paranormal.screen.elements.button.PowerTabButton;
...
public class PowersScreen extends Screen {
	...
	public void render(DrawContext drawContext, int mouseX, int mouseY, float partialTicks) {
		...
		for (Element e : children()){
			if (e instanceof PowerSlotButton slotButton && slotButton.isMouseOver(mouseX, mouseY) && slotButton.getPower() != null){
				...
				drawContext.fill(mouseX + 4, mouseY + 4, mouseX + 4 + width, mouseY + 4 + tooltipHeight, 0xFF602c2c);
				drawContext.fill(mouseX + 4, mouseY + 4, mouseX + 4 + width, mouseY + 4 + 1, 0xFFde9e41);
				drawContext.fill(mouseX + 4, mouseY + 4, mouseX + 4 + 1, mouseY + 4 + tooltipHeight, 0xFFde9e41);
				drawContext.fill(mouseX + 4, mouseY + 5 + tooltipHeight, mouseX + 4 + width, mouseY + 4 + tooltipHeight, 0xFFde9e41);
				drawContext.fill(mouseX + 4 + width, mouseY + 4, mouseX + 5 + width, mouseY + 4 + tooltipHeight, 0xFFde9e41);

				for (int i = 0; i < lines.size(); i++) {
					drawContext.drawText(client.textRenderer, lines.get(i), mouseX + 7, mouseY + 7 + client.textRenderer.fontHeight * i,
							0xFFFFFF, false);
				}
			}
		}
	...
	public void renderBackground(DrawContext drawContext, int mouseX, int mouseY, float delta) {
		...
		drawContext.drawTexture(TEXTURE, x, y, 52, 173, 0, 0, 52, 173, 256, 256);
		drawContext.drawTexture(TEXTURE, x + 56, y, 199, 144, 52, 0, 199, 144, 256, 256);
	}
	...
	public static class PowerScrollBar extends ClickableWidget implements Drawable, Element {
		...
		protected void renderWidget(DrawContext drawContext, int mouseX, int mouseY, float delta) {
			if (powersScreen != null && !powersScreen.powers.isEmpty()) {
				...
				drawContext.drawTexture(PowersScreen.TEXTURE, getX(), getY() + scrollPosition, 15, 19, 226, 144, 15, 19, 256, 256);
