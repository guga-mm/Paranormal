...
public class PexHud implements HudRenderCallback {
	...
	public void onHudRender(GuiGraphics guiGraphics, float tickDelta) {
		...
		if (!client.options.hudHidden && client != null) {
			...
			guiGraphics.drawTexture(TEXTURES, width - 95, height - 9, 3, 24, 92, 5);
			...
			guiGraphics.drawTexture(TEXTURES, width - 95, height - 9, 3, 29, (int) barFilled, 5);

			String s = playerData.getNexPercentage() + "%";
			guiGraphics.drawShadowedText(client.textRenderer, s,
			...
			if (client.interactionManager.hasStatusBars()){
				...
				for (int i = 0; i < max; i++){
					...
					guiGraphics.drawTexture(TEXTURES, l1, i2, 125, 0, 9, 9);
				}
				...
				for (int i = amount - 1; i >= 0; --i) {
					...
					guiGraphics.drawTexture(TEXTURES, l1, i2, 116, 0, 9, 9);
				}
			}
		}
	}
